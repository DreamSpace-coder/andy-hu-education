...
-   "What are the vendors that in the same city and state?"
...
This is essentially stacking different intermediate table results one on top of another: - Each result set must return the same number of columns. - The corresponding columns in each result set must have compatible data types. - The column names in the final result set are taken from the first SELECT clause.
